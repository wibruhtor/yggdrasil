...
macro_rules! scopes {
    (
        ...
        $(
            $(#[$docs])*
            pub const $name: Scope = Scope($scope);
        )+
        }
    }
}
...
scopes! {
    (ANANLYTICS_READ_EXTENSIONS, "analytics:read:extensions");
    (ANANLYTICS_READ_GAMES, "analytics:read:games");
    (BITS_READ, "bits:read");
    ...
    (CHANNEL_READ_GUEST_START, "channel:read:guest_star");
    (CHANNEL_MANAGE_GUEST_START, "channel:manage:guest_star");
